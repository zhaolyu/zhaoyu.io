...
- **Preview verification is live** (added 2026-09-01, outside the original spec). `.github/scripts/smoke-preview.sh` runs in the deploy job against the real preview deployment through a Cloudflare Access service token, and is blocking as of its first PASS. It is the only thing that proves `static/_redirects` actually fires at the edge; `redirects.test.ts` proves the rules are coherent, not that Cloudflare honours them. Setup and renewal are in `.github/workflows/CLOUDFLARE_SETUP.md`.
