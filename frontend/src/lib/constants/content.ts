...
const stealthHero: HeroContent = {
  ...
  bio: 'Senior Manager, Engineering across CNBC Web & Make It divisions—directing a 20-engineer organization executing the platform rebuild of CNBC.com ([redacted]–55M ARR, [redacted] subscribers), while staying hands-on in core architecture. Nine years of performance-first engineering: Isomorphic Akamai Edge for [redacted] monthly users, [redacted], zero-downtime infrastructure migrations, and governed AI integration.',
  cta: CTA,
  motto: MOTTO,
};
...
export const performanceMetrics: PerformanceMetric[] = [
  { label: 'LCP', value: '1.1s', sublabel: 'CNBC.com · [redacted]' },
  { label: 'Monthly Users', value: '[redacted]', sublabel: 'CNBC Reach' },
  { label: 'Akamai TTFB', value: '[redacted]', sublabel: 'EdgeWorkers · Global' },
  { label: 'Cache Hit Rate', value: '[redacted]', sublabel: 'Akamai Edge Layer' },
  { label: 'Production Years', value: '9+', sublabel: 'Shipped at Scale' },
  { label: 'Core Web Vitals', value: '[redacted]', sublabel: 'Lighthouse Score' },
];
  ...
  projects: [
    {
      ...
      description:
        'Architected the migration of CNBC.com from a legacy client-side monolith to a high-performance Isomorphic Akamai Edge architecture. Business and rendering logic moved to Akamai EdgeWorkers — executing at the network edge, not origin. The result: [redacted] global TTFB and a [redacted] during market-moving events with [redacted] concurrent users and zero downtime.',
    ...
    },
    {
      ...
      description:
        "Non-deterministic output requires deterministic UI. Engineered the frontend architecture for CNBC's first user-facing AI assistant — currently a [redacted] beta pilot, not a site-wide surface — solving the HCI paradox of maintaining user trust when data is streaming and non-deterministic. Built a latency masking layer for token rendering at 60fps and a real-time citation engine that maps AI-generated tokens to verified CNBC sources.",
...
function platformRebuildProject(flags: PositioningFlags): BuilderProject {
  ...
  return {
    ...
    status: 'active',
    metrics: [
      { label: 'Subscriber ARR', value: '[redacted]–55M' },
      loud
...
const staticBuilderProjects: BuilderProject[] = [
  {
    ...
    category: 'professional',
    description:
      "[redacted] and technical bridge for CNBC's first user-facing AI assistant. Spec'd and shipped the multi-step agentic pipeline ([redacted] → MCP data retrieval → [redacted] → response generator) in 4 months to a [redacted] beta cohort, establishing the architectural blueprint for future conversational surfaces.",
...
const surfaceOwnershipParagraph =
  "Today that means owning both ends of the strategic surface area: I was the [redacted] for CNBC's AI financial assistant — now validated with a [redacted] beta cohort — and I lead the team shipping the next-generation video experience, the property's revenue engine.";
...
export function buildNarrativeBio(flags: PositioningFlags = FEATURE_FLAGS): NarrativeBio {
  return {
    ...
    paragraphs: [
      ...
      'As AI Integration Lead, I formalize the standards and PR quality gates (SonarQube, lint, Jest test automation) governing how 20+ engineers across 3 web teams integrate AI tools like Cursor into production codebases, delivering a [redacted] increase and [redacted] high-severity defects. The philosophy underneath it is "Plan-first" execution — AI as a disciplined velocity multiplier inside strict security and compliance guardrails, never as an unreviewed author.',
...
export function buildSocialDescriptions(
  ...
  return flags.goLoudPositioning
    ? {
        meta: "Senior Manager, Engineering at Versant (CNBC Web & Make It). [redacted] of CNBC's AI financial assistant ([redacted] beta pilot); lead the next-gen web video platform. 20-engineer org, [redacted] users, [redacted].",
        twitter:
          'Senior Manager, Engineering at Versant (CNBC Web & Make It). AI financial assistant architecture, next-gen web video, and edge performance at [redacted] users.',
      }
    : {
        meta: 'Senior Manager, Engineering at Versant (CNBC Web & Make It) — directing a 20-engineer organization through the platform rebuild of CNBC.com, while staying hands-on in core architecture. [redacted] users, [redacted].',
        twitter:
