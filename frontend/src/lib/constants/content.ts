  ...
  notes: [
    ...
    },
    {
      ...
      content: [
        ...
        'So I built OB1, a personal exocortex of 1,700 claims, to store judgment the way it must be retrieved. A claim is one assertable proposition, named by a filename that states it, hand-typed as claim, pattern, tension, or anti-pattern — <strong>nothing derives that type, which is why it carries judgment</strong>: a thing is an anti-pattern because someone decided it was. Retrieval embeds the query, ranks with pgvector, then expands one hop along the wiki-links, so a claim reached by a <code>parallels</code> edge surfaces even when its similarity is low; 2,777 of the 8,281 edges cross domains, the ones worth the hop. MCP exposes it to every agent session. On 15 August a reduce pass was about to generalize a second claim about legibility inverting felt value — retrieval returned <code>prevented-loss-invisibility</code>, already established on an external source, and the output became an enrichment instead of a duplicate. <strong>The judgment you have to remember to look up is judgment you do not have.</strong>',
      ],
    },
    {
