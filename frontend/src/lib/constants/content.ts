...
export const notesData: NotesData = {
  notes: [
    ...
    },
    {
      ...
      content: [
        "I built most of this site through Claude Code as a deliberate workflow: the components, the type errors that blocked deploys, and the notes themselves all went through a loop of prompting, reviewing, and committing. The obvious claim to make about that is speed, and the best available evidence does not support it. <a href='https://arxiv.org/abs/2507.09089' target='_blank' rel='noopener'>METR ran a randomized controlled trial on sixteen experienced open-source developers across 246 real tasks in their own repositories</a> and measured them 19% <em>slower</em> with AI tools than without. The same developers estimated afterward that they had been 20% faster.",
        "What changed for me was the filter. When a first version costs little enough, an idea stops being killed at the worth-building stage and starts being killed by contact with something running, which is a later filter and a much better one. The compounding is in the attempt count: an engineer who runs ten experiments in a week has ten results to reason from, and those results are what carry into the next decision. The caveat travels with the claim. METR's own <a href='https://metr.org/blog/2026-02-24-uplift-update/' target='_blank' rel='noopener'>February 2026 update</a> reports that developers are likely more sped up now than in early 2025, while selection effects leave the size of that increase uncertain. <strong>Count the things you tried, not the speed you felt.</strong>",
      ],
    },
    {
      slug: 'the-url-is-the-source-of-truth',
      title: 'The URL Is the Source of Truth, and the Front End Is a Distributed System',
      date: 'Jul 11, 2026',
      dateISO: '2026-07-11',
      dateModified: '2026-09-01',
      ...
      content: [
        'Filter a dashboard, refresh the page, and the filter is gone. Send the link to a colleague and they see a different view. The usual diagnosis is that the state was never persisted, and the usual fix is a client store, which handles the reload and misses the mechanism. Holding that filter in the URL is <strong>single-leader replication</strong>: one authoritative location owns the value, every view is a follower, and rendering is a read. Holding it in a store, plus the derived copies three components keep, is multi-leader replication. Most of what <em>Designing Data-Intensive Applications</em> says about multi-leader setups, inside its replication chapter, is about handling write conflicts, because that is where the difficulty always lands: concurrent writers, no total order, and no log to replay when the copies disagree.',
        "The browser sits behind a network nobody controls, so a request that times out has not necessarily failed, and a payment the reader submits twice still has to land once. The IETF has a <a href='https://datatracker.ietf.org/doc/draft-ietf-httpapi-idempotency-key-header/' target='_blank' rel='noopener'>working draft for an <code>Idempotency-Key</code> header</a> because that is a protocol-level problem rather than an application quirk, and the rule it encodes is that a client must always be able to retry safely. Rendering at the edge is a blast-radius decision before it is a performance one: a region that degrades during a market-moving event should be routed around, not queued behind. None of this is new work. It is replication, retry semantics, and failure domains, arriving in a codebase where nobody uses those words. <strong>You cannot reuse a solved problem you do not recognize.</strong>",
      ],
    },
    {
