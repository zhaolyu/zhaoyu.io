...
<header class={headerClasses}>
  <div class="max-w-7xl mx-auto px-6 flex items-center justify-between">
    <a href="/" onclick={(e) => scrollToTop(e)} class="group flex items-center gap-2">
      ...
      <div
        class="h-5 w-2 bg-blue-500 opacity-0 group-hover:opacity-100 transition-opacity duration-100 animate-pulse"
      ></div>
    </a>
    ...
    <nav class="hidden md:flex items-center gap-6">
      {#each navLinks as link (link.href)}
        ...
        >
          {link.name}
          <span
            class="absolute -bottom-1 left-0 w-0 h-px bg-blue-500 transition-all group-hover:w-full"
          ></span>
        </a>
      {/each}
    </nav>
  ...
  >
    ...
    >
      <div class="mobile-menu-header">
        ...
        <button onclick={closeMenu} class="mobile-menu-close" aria-label="Close menu" type="button">
